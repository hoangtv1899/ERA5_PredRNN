...
def get_fc_array(dt):
    ...
    basepath = '/pscratch/sd/h/hvtran/graphcast/'
    ...
    temp_array = []
    for j in range(4):
        f0i = basename+di.strftime('%Y-%m-%d-%H')+'_'+str(j)+'.nc'
        f0i = basepath+f0i
        if not os.path.isfile(f0i):
            return None
        ds0 = xr.open_dataset(f0i)
        time_step_arr = np.zeros((4,721,1440))
        for data in ds0:
            if data == '10m_u_component_of_wind':
                time_step_arr[0,...] = ds0[data].values[0,0,...]
            elif data == '10m_v_component_of_wind':
                time_step_arr[1,...] = ds0[data].values[0,0,...]
            elif data == 'mean_sea_level_pressure':
                time_step_arr[2,...] = ds0[data].values[0,0,...]
            elif data == '2m_temperature':
                time_step_arr[3,...] = ds0[data].values[0,0,...]
...
def cal_stats(which_month, offset, existing_acc_ds=None, existing_rmse_ds=None):
    ...
    if existing_acc_ds is not None:
        if np.datetime64(curr_date) in existing_acc_ds.time:
            return existing_rmse_ds.sel(time=slice(curr_date,curr_date+ timedelta(hours=12))),\
                    existing_acc_ds.sel(time=slice(curr_date,curr_date+ timedelta(hours=12)))
    ...
    tempe_arr = pred_arr[:,3:4,...]
    tempe_arr = tempe_arr[[1,2,3,0],...]
    pred_arr[:,3:4,...] = tempe_arr
    #pred_arr = pred_arr[[1,2,3,0],...]
    ...
    obs_xr = np2xr(valid_arr1,var_dict,lon,lat,time_ds,variables)
...
if os.path.isfile('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_acc_'+str(which_month)+'.nc'):
    existing_acc_ds = xr.open_dataset('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_acc_'+str(which_month)+'.nc')
    existing_rmse_ds = xr.open_dataset('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_rmse_'+str(which_month)+'.nc')
    os.remove('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_acc_'+str(which_month)+'.nc')
    os.remove('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_rmse_'+str(which_month)+'.nc')
else:
...
for i in range(1,monthrange(2023,which_month)[1]):
    ...
    if rmsei is not None:
        ...
        temp_acc.to_netcdf('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_acc_'+str(which_month)+'.nc', mode='w')

        temp_rmse = xr.concat(part_rmse, 'time')
        temp_rmse.to_netcdf('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_rmse_'+str(which_month)+'.nc', mode='w')
        print(i)

final_acc = xr.concat(part_acc, 'time')
final_acc.to_netcdf('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_acc_'+str(which_month)+'.nc', mode='w')

final_rmse = xr.concat(part_rmse, 'time')
final_rmse.to_netcdf('/global/homes/h/hvtran/haoli/ERA5_PredRNN/validation/results/GC_2023_rmse_'+str(which_month)+'.nc', mode='w')
